...
int main(){
    ...
    //Scoreboard
    int scoreboard_reg; //holds the dest reg of the current executing instruction
    INSTRUCTION_TOKEN scoreboard_instr; //holds the current executing instruction
	...
	// Handle execute of instruction
        ...
        if (execute_inst != NULL) { 
            int address;
            scoreboard_reg = execute_inst->reg_dst; //if not dstreg, -1
            scoreboard_instr = execute_inst; //if not, NULL
	...
	// Handle instruction queue and scheduling (scoreboard)
        ...
        if (frontend_flush == true){
            ...
            if (instruction_queue->IsEmpty() == false){
                ...
                if (scoreboard_reg != -1 && (schedule_inst->reg_srca == scoreboard_reg || schedule_inst->reg_srcb == scoreboard_reg)){

                    //there is a dependency, annotate in this instruction
                    schedule_inst->SetDependency(scoreboard_instr);
                    schedule_inst = NULL;
                }
                else {
